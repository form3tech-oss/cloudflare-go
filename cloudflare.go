...
func (api *API) makeRequestWithAuthTypeAndHeaders(ctx context.Context, method, uri string, params interface{}, authType int, headers http.Header) ([]byte, error) {
	...
	if params != nil {
		...
		case []byte:
			// If params is []byte then pass through to the request body untouched
			jsonBody = v
		case io.Reader:
			// If params is already an io.Reader, e.g. from a multipart form then consume into bytes
			jsonBody, err = ioutil.ReadAll(v)
			if err != nil {
				return nil, errors.Wrap(err, "error reading request body")
			}
		default:
			// Anything else we try to JSON encode
